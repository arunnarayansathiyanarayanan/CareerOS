-- Job postings, demand snapshots, and related enums for skill intelligence pipeline.
...
CREATE TABLE IF NOT EXISTS public.user_skill_graph (
  ...
  proficiency SMALLINT NOT NULL CHECK (proficiency >= 1 AND proficiency <= 5),
...
CREATE TABLE IF NOT EXISTS public.skill_gap_scores (
  ...
  role TEXT NOT NULL,
  city TEXT NOT NULL,
  gap_score SMALLINT NOT NULL CHECK (gap_score >= 0 AND gap_score <= 100),
  ranked_skills JSONB NOT NULL DEFAULT '[]'::jsonb,
  computed_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
